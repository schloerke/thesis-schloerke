...
The R package \pkg{plyr} implemented the split-apply-combine approach for many kinds of data shapes: \rinline{array}, \rinline{list}, \rinline{vector}, and \rinline{data.frame}.  \pkg{dplyr} has many specific routines to interact with a \rinline{data.frame}.  Examples will be using the \pkg{dplyr} package functions.
...
Cognostics can be simple summary statistics such as \rinline{mean} or \rinline{median}, or can be meta data information such as a URL or census information.  Tukey and Tukey first proposed calculating univariate metrics for scatter plots called scagnostics \cite{Tukey1985} as a way to describe a scatterplot.  Wilkinson et. al. implemented Tukeys' scagnostic definitions in the R package \pkg{scagnostics} (CITE scagnostics) \cite{scagnostics}. Scagnostics can be repurposed as cognostics when applied to every panel containing a scatterplot.  These cognostic groupings may then be compared, filtered, and sorted accross different subset panels.
