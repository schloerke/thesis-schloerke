...
The example below uses the \rinline{gapminder} data set.  The \rinline{gapminder} data set contains an ``Excerpt of the Gapminder data on life expectancy, GDP per capita, and population by country.''  The 142 countries have data from 1952 to 2007.  The example will only use the life expectancy variable.
