...
\trelliscopejs~is an R package used to visualize data with many conditioning combinations.  \trelliscopejs~conditions on columns within the data set and displays a plot, image, or html object for each plot in an independent display panel.
...
Summary statistics are great in interpreting information using less data.  However, summary statistics, by their nature, do not convey the full story. % TODO wording.  don't say ``full story''
...
Using the \rinline{gapminder} data set in figure \ref{fig:trelliscopejs}, life expectancy is explore over time with the supplimentary metrics of minimum and maximum life expectancy.
...
@
<< trelliscopejs, screenshot.opts = list(vwidth = 700, vheight = 525), fig.cap = "Full data exploration with manually added cognostics are made available in a \\trelliscopejs~widget." >>=
gapminder %>%
  group_by(country, continent) %>%
  # condense the data
  tidyr::nest() %>%
