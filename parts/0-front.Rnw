...
\begin{abstract}
  ...
  % problem is important?
Statistical visualization of large-scale data sets has become an increasingly essential task in the era of \emph{big data}.
  ...
  % what have people done?
  %   2-3 sentences
Building on the concept of a \emph{grammar of graphics}, the R package \ggplot~laid the foundation for creating customized data visualizations within R.  The distributed multi-panel display provided by the \trelliscopejs~R package further enables \ggplot~visualizations to be scaled up to large data sets.
  % what is missing?
  %   However, ...
  However, there are restrictions and lack of communication among the development of both packages. For instance, \ggplot~does not allow for both continuous and discrete scales to be displayed in a single plot matrix, while \trelliscopejs~requires all cognostics used for panel interactions to be supplied manually.  With the data being stored in R, many web data queries scale linearly with data size rather than remaining constant with query requirements.
  % we address this missing by...?
  %   In this work, we fill this gap by BLAH using packages A, B, C.
  In this work, I fill these gaps by enhancing and developing three R packages: \GGally, \autocogs, and \gqlr.
  % 3x more sentences describing each package
  %   ggduo does a gen pairs plot
  %   autocogs does X and addresses Y
  %   gqlr does A and bridges the gap of B
  Within \GGally, the function \rinline{ggduo} produces a generalized plot matrix for two-grouped data which displays all combinations of two sets of variables within a data set.
  Complimenting \trelliscopejs, the package \autocogs~aids in visualization panel exploration~by automatically generating standard cognostic groups for each \trelliscopejs~panel.
  Finally, the package \gqlr~implements a GraphQL server to minimize the required number of queries needed to perform data extraction while allowing the user to benefit from the rapid development iteration time of R and production iteration time of GraphQL.
  % stress the importance of the contribution
  %   These three packages will address issues....
  Together, these three packages address the increasing scale and complexity of today's data visualization demands.
\end{abstract}
