...
\section{R Language}
  ...
  R has over 11,000 active, user submited packages (as of Sept. 2017, \url{https://cloud.r-project.org/web/packages/index.html}) and countless more on Github (\url{github.com}).  Among these packages are \pkg{GGally}, \pkg{gqlr}, \pkg{autocogs}, and \trelliscopejs.  These are a few of the R packages I will describe in greater detail.
...
\ggplot, a data visualization R package, is based on a \emph{Layered Grammar of Graphics} (\cite{WickhamHadley2010ALGo}).  During Dr. Wickham's studies at Iowa State University, he statistically consulted with many students and professors on how to produce visualizations for their studies. Dr. Wickham noticed many clients had trouble producing plots quickly and difficulty understanding how the plot was made.  This provided plenty of motivation to produce \ggplot, basing it on \emph{The Grammar of Graphics}.
