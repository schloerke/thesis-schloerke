...
\section{R Language}
  ...
  One of the most notible R packages is \pkg{ggplot2}.  It was originally written by Hadley Wickham in 2005.  Since then, \pkg{ggplot2} has become one of the most impactful R packages (\url{http://depsy.org/package/r/ggplot2}).
...
% What is ggplot2
  ``\pkg{ggplot2} is a system for declaratively creating graphics, based on \emph{The Grammar of Graphics}. You provide the data, tell ggplot2 how to map variables to aesthetics, what graphical primitives to use, and it takes care of the details.'' (\url{https://github.com/tidyverse/ggplot2})
...
\pkg{ggplot2} is based on a \emph{Layered Grammar of Graphics} (\cite{layered-grammar}).  During Dr. Wickham's studies at Iowa State University, he statistically consulted with many students and professors on how to produce visualizations for the studies. Dr. Wickham noticed many clients had trouble producing plots quickly and difficulty  understanding how the plot was made.  This gave plenty of motivation to produce \pkg{ggplot2} and base it on \emph{The Grammar of Graphics}.
...
<<>>=
p <- ggplot(tips, aes(total_bill, tips)) +
  geom_point()
p
@

Typically, each layer of the plot uses the same original data source to help better explore the data frame.  Multi-layered plots may use multiple data sources where it would not make sense to combine data sources.  One of the data sources will be displayed on every panel of the plot.
...
p + geom_abline(data = tip_lm_dt, aes(slope = slope, intercept = intercept))
@
...
All plot layers to not need to exist at the inception of plot object.  Each layer of a \pkg{ggplot2} plot can be added one by one to the original plot object.  The simplist of plot objects consists of a default dataset and a default set of aesthetics.
...
TODO clean up this part.  add in facet section?
  ...
  plot objects are not rendered until print method is called
    allows for modularization of creating a plot.
    allows for simplification of interface as independent functionality can be separated into different functions.
...
Faceting in ggplot2 is when conditioning variables are used to divide data into independent data subsets which produce similar plots.  This technique commonly refered to as small multiples (CITE).  The same plots are displayed, but each plot contains an independent subset of the data.  Typically, only one or two conditioning variables are used, but any number of variables may be used when faceting a \pkg{ggplot2} plot.
