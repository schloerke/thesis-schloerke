...
autocogs::auto_cog("univariate_counts", americas$lifeExp)
autocogs::auto_cog("bivariate_counts", americas$year, americas$lifeExp)
@
...
autocogs::auto_cog("pairwise_counts", americas$country, round(americas$lifeExp))
@
% autocogs::auto_cog("grouped_counts", gap_max$country, gap_max$continent)
...
autocogs::auto_cog("histogram_counts", americas$lifeExp)
@
...
autocogs::auto_cog("square_counts", americas$year, americas$lifeExp, bins = 6)
@
...
autocogs::auto_cog("hex_counts", americas$year, americas$lifeExp, bins = 6)
@
...
\includegraphics[width=\textwidth]{./dot/ggplot_cognostics.pdf}
...
While this amount of information is a little overwhelming, keep in mind the goal of the autocogs package is to provide as many ways to filter and sort data that can not readily be seen.
...
gapminder %>%
  group_by(country, continent) %>%
  do(
    panel = ggplot(., aes(year, lifeExp)) + geom_point() + geom_smooth(method = "lm")
  ) ->
country_model
