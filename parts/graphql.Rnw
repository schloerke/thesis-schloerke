...
union NotAPlant = Person | Dog | Cat | Shark
\end{graphqlcode}
...
Every object in the Schema definition can be queried.  A query on an object type must include at least one field.  Object types are not considered Leafs in the Schema definition tree.
...
The whole query string is submitted to the GraphQL server.  This allows for complex and deeply nested queries in a single request.  Using the query below, we can query for all pets owned by the same owners as Dutchess the cat.
...
<<named_type_dog>>=
gqlr:::NamedType$new(name = gqlr:::Name$new(value = "Dog"))
gqlr:::NamedType$new(name = "Dog")
@
...
There are many existing and HTML request handlers and authenticators existing for R.  $gqlr$ was built to handle GraphQL server requests and leave the url routing and authentication to better suited packages.
