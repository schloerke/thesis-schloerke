...
union NotAPlant = Person | Dog | Cat | Rat
\end{graphqlcode}

Results being returned could either be a \graphqlinline{Person}, \graphqlinline{Dog}, \graphqlinline{Cat}, or \graphqlinline{Rat} type.  All of the types included in \graphqlinline{NotAPlant} do not share a common field, but could feasibly be returned in the same field of information.
