...
<< original_pairs, fig.cap = "Stock R graphics scatterplot matrix displaying the \\rinline{tips} dataset." >>=
...
<< gpairs_intro, fig.cap = "Same \\rinline{tips} data, but the plot matrix is displayed using the \\pkg{gpairs}~R package" >>=
gpairs::gpairs(tips[, c("total_bill", "tip", "smoker", "day")])
@

<< ggpairs_intro, fig.cap = "Same \\rinline{tips} data, but the plot matrix is displayed using the \\pkg{GGally}~R package" >>=
...
<< add_custom_plot, fig.width = 5, fig.height = 3.75, out.width = "4in", out.height = "3in", fig.cap = "Full \\ggplot plot object from the second row and first column." >>=
...
<< grid_extra, fig.cap = "Two fully displayed \\ggplot~plot objects arranged using \\pkg{gridExtra}. Duplicate axes and labels are present.  The $X$ axis does not align as the plots are treated independently." >>=
p1 <- pm[1,4]
p2 <- pm[2,4]
...
As a quick example, we may look at Australian students' fifth grade Math, Reading, and Science scores against their $gender$ and how many hours of $homework$ each student completed each week.
...
<< ggduo_types, fig.width = 5, fig.height = 3.75, out.width = "4in", out.height = "3in", fig.cap = "Two grouped plot matrix using \\rinline{ggduo} with no upper or lower triangle areas." >>=
# default
ggduo(
  tips,
  c("size", "smoker", "day"),
...
@
<< ggduo_types2, fig.width = 5, fig.height = 3.75, out.width = "4in", out.height = "3in", fig.cap = "Updating the combination types in a \\rinline{ggduo} plot matrix." >>=
...
<< user_defined, fig.width = 5, fig.height = 3.75, out.width = "4in", out.height = "3in", fig.cap = "Use a custom function to display a plot within a \\rinline{ggduo} plot matrix." >>=
...
<< stats_ts, fig.width = 5, fig.height = 3.75, out.width = "4in", out.height = "3in", fig.cap = "Stock R time series plot.  All variables are displayed on the same axis." >>=
