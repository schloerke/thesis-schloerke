...
\subsection{Pairs plot}
  ...
  A pairs plot, originaly refered to as a ``draftsman's plot'' \cite{GraphicalMethods},  violates all three principles of a small multiple.
...
\begin{itemize}
  \item continuous vs. continuous. A scatter plot a continuous vs. continuous plot.
...
% TODO
% add conclusion
  % discribe how ggduo extended the capabilities fo the pairs plot while maintaining modularity.
  %
  % It was extended further for time series plot and model diagnostic plot matrix
