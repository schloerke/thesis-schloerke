...
\subsection{Pairs Plot}
  ...
  As the last chapter mentioned, \ggplot is designed to allow plots to be built layer by layer and not rendered until a final print command is executed.  \ggplot plot objects has some implementation rules it follows when facet'ing a plot.
