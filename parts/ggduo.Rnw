...
To achieve the delay between plot matrix inception and displaying the plot matrix, a \rinline{ggmatrix} object is used.  This object holds all the necessary information to produce a plot matrix:  the number of rows and columns, label information, \pkg{ggplot2} display theme information,  how plot strips should be displayed, default plot data, and individual plot information.
