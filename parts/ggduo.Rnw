...
<< gpairs_intro, fig.cap = "\\rinline{gpairs}~plot matrix from the \\pkg{gpairs}~R package displaying the \\rinline{tips} dataset." >>=
gpairs::gpairs(tips[, c("total_bill", "tip", "smoker", "day")])
@

<< ggpairs_intro, fig.cap = "\\rinline{ggpairs}~plot matrix from the \\GGally~R package displaying the \\rinline{tips} dataset." >>=
...
%
%
% Link up with the javascript packages for interactive plot matrices. Would be great to add a one more line of code to turn it into an interactive plot.
