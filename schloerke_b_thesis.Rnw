...
\usepackage{hyperref} % actual links % TODO make colors less weird / red
...
\chapter{\rinline{ggduo}: Generalized Pairs Plot for Two Grouped Data} \label{chapter:ggduo}
...
\chapter{\gqlr: An R Server GraphQL implementation} \label{chapter:gqlr}
